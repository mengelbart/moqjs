...
export class Session {
  ...
  static async connect(url: string, serverCertificateHash: string) {
    ...
    try {
      ...
      console.log('could not connect WebTransport using serverCertificateHashes, trying wihtout', err);
      conn = new WebTransport(url);
    }
    if (!conn) {
    ...
    const decoderStream = new ReadableStream(new ControlStreamDecoder(cs.readable));
  ...
  async readIncomingUniStream(stream: WebTransportReceiveStream) {
    ...
    const messageStream = new ReadableStream<ObjectStream>(new ObjectStreamDecoder(stream));
    ...
    for (;;) {
      ...
      console.log('writing to subscription', this.subscriptions.get(value.subscribeId));
      const writer = this.subscriptions.get(value.subscribeId)!.subscription.writable.getWriter();
  ...
  async subscribe(namespace: string, track: string): Promise<ReadableStream> {
    ...
    await this.controlStream.send(new SubscribeEncoder({
