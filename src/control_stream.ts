import { CURRENT_SUPPORTED_DRAFT, ClientSetupEncoder, MessageType, ParameterEncoder } from "./messages";
...
export class ControlStream {
  ...
  async handshake() {
    ...
    await writer.write(new ClientSetupEncoder({
      type: MessageType.ClientSetup,
      versions: [CURRENT_SUPPORTED_DRAFT],
      parameters: [new ParameterEncoder({type: 0, value: new Uint8Array([0x2])})],
    }));
    writer.releaseLock();
